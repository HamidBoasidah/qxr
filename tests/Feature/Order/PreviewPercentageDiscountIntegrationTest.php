...
/**
 * Integration Test: Order Preview with Percentage Discount
 * 
 * **Validates: Requirements 5.7, 6.3**
 * 
 * This test verifies the complete end-to-end preview flow from HTTP request
 * to response when percentage discount offers are applied.
 */
class PreviewPercentageDiscountIntegrationTest extends TestCase
{
    use RefreshDatabase;

    /**
     * Test: Complete preview flow with percentage discount
     * 
     * **Validates: Requirements 5.7, 6.3**
     * 
     * Verifies that:
     * 1. Preview request is accepted with HTTP 200
     * 2. Percentage discount is calculated correctly: (min_qty × unit_price × reward_value / 100) × multiplier
     ...
     */
    #[Test]
    public function preview_succeeds_with_percentage_discount(): void
    {
        ...
        // final total = 2000.00 - 200.00 = 1800.00

        $previewData = [
            'company_id' => $company->id,
            'notes' => 'Test preview with percentage discount',
            'items' => [
                [
                    'product_id' => $product->id,
                    'qty' => 200
        ...
        $this->assertEquals('Test preview with percentage discount', $responseData['notes']);

        // Assert: Items count
        $this->assertCount(1, $responseData['items']);

        // Assert: Item details with percentage discount
        ...
        $this->assertEquals('10% Discount on 100+ units', $item['offer_title']);
        ...
        $this->assertEquals('Test preview with percentage discount', $cachedPreview['notes']);
    ...
    /**
     * Test: Preview with percentage discount and fractional multiplier
     * 
     * **Validates: Requirements 5.7, 6.3, 5.4**
     * 
     * Tests that multiplier is correctly floored when qty doesn't evenly divide by min_qty
     */
    #[Test]
    public function preview_with_percentage_discount_floors_multiplier(): void
    {
        // Arrange
        $company = User::factory()->create(['user_type' => 'company', 'is_active' => true]);
        $customer = User::factory()->create(['user_type' => 'customer', 'is_active' => true]);
        ...
        // final total = 625.00 - 75.00 = 550.00
        ...
        $this->assertEquals(550.00, $item['final_total']);
        $this->assertEquals($offer->id, $item['selected_offer_id']);

        // Assert totals
        $this->assertEquals(625.00, $responseData['subtotal']);
        $this->assertEquals(75.00, $responseData['total_discount']);
        $this->assertEquals(550.00, $responseData['final_total']);
    }

    /**
     * Test: Preview with multiple items having percentage discounts
     * 
     * **Validates: Requirements 5.7, 6.3, 5.1**
     */
    #[Test]
    public function preview_with_multiple_percentage_discounts(): void
    {
        // Arrange
        $company = User::factory()->create(['user_type' => 'company', 'is_active' => true]);
        $customer = User::factory()->create(['user_type' => 'customer', 'is_active' => true]);
        ...
        // Product 1: qty=100, multiplier=1, discount=(100*10*10)/100=100, subtotal=1000, final=900
        // Product 2: qty=100, multiplier=2, discount_per_block=(50*20*20)/100=200, total_discount=400, subtotal=2000, final=1600

        $previewData = [
            'company_id' => $company->id,
            'notes' => 'Multiple percentage discounts',
            'items' => [
                [
                    'product_id' => $product1->id,
                    'qty' => 100
    ...
    /**
     * Test: Preview with percentage discount below minimum quantity
     * 
     ...
     */
    #[Test]
    public function preview_with_percentage_discount_below_min_qty_applies_no_offer(): void
    {
        // Arrange
        $company = User::factory()->create(['user_type' => 'company', 'is_active' => true]);
        $customer = User::factory()->create(['user_type' => 'customer', 'is_active' => true]);
