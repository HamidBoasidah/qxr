<template>
    <div class="overflow-hidden">
        ...
        <div class="max-w-full overflow-x-auto">
            <table class="w-full min-w-full">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
                            <div
                                ...
                                @click="sortBy('name')"
                            >
                                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('tags.name') }}</p>
                        ...
                        <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
                            <div
                                ...
                                @click="sortBy('slug')"
                            >
                                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('tags.slug') }}</p>
                                <span class="flex flex-col gap-0.5">
                                    <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill=""/></svg>
                                    <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0.455342 6.94929 0.786731L4.40962 4.41483Z" fill=""/></svg>
                                </span>
                        ...
                        <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
                            <div class="flex items-center justify-between w-full cursor-pointer">
                                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('tags.tagType') || 'Type' }}</p>
                            </div>
                        </th>
                        <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
                ...
                <tbody>
                    <tr
                        v-for="tag in paginatedData"
                        :key="tag.id"
                        :class="{ 'bg-gray-50 dark:bg-gray-900': tag.selected }"
                    >
                            ...
                            >
                                <span class="relative">
                                    ...
                                    <span
                                        ...
                                        class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                                    >
                                        <span :class="tag.selected ? '' : 'opacity-0'">
                                            <svg
                                                width="12"
                                                height="12"
                                                viewBox="0 0 12 12"
                        ...
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ tag.name }}</p>
                        </td>
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ tag.slug }}</p>
                        </td>
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <span
                                class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium"
                                :class="{
                                    'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': tag.is_active,
                                    'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !tag.is_active,
                                }"
                            >
                        ...
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ t('tags.types.' + (tag.tag_type || '')) || tag.tag_type }}</p>
                        </td>
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <label :for="'toggle-' + tag.id" class="cursor-pointer">
                                <div class="relative">
                                    <input
                                        type="checkbox"
                                        :id="'toggle-' + tag.id"
                                    ...
                                    <div
                                        ...
                                        :class="tag.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
                                    ></div>
                                    <div
                                        :class="[tag.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']"
                        ...
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <div class="flex items-center w-full gap-2">
                                ...
                                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                                    <button
                                        :disabled="!canDelete"
                                        @click="handleDeleteClick(tag.id)"
    ...
    <DangerAlert
        ...
        :message="t('messages.deleteTagConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['tags.create', 'tags.store', 'tags.add']))
const canView = computed(() => hasAnyPermission(['tags.view', 'tags.show', 'tags.read']))
const canEdit = computed(() => hasAnyPermission(['tags.update', 'tags.edit']))
const canDelete = computed(() => hasAnyPermission(['tags.delete', 'tags.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ tags: Object })

const search = ref('')
const sortColumn = ref('name')
const sortDirection = ref('asc')
const currentPage = ref(props.tags?.current_page ?? 1)
const perPage = ref(props.tags?.per_page ?? 10)
const selectAll = ref(false)

function goToCreate() {
    router.visit(route('admin.tags.create'))
}

function goToView(id) {
    router.visit(route('admin.tags.show', id))
}
...
function confirmDelete() {
    if (tagToDeleteId.value) {
        router.delete(route('admin.tags.destroy', tagToDeleteId.value), {
            onSuccess: () => {
                success(t('tags.tagDeletedSuccessfully'))
                closeDeleteModal()
            },
            onError: () => {
                error(t('tags.tagDeleteFailed'))
                closeDeleteModal()
            },
            preserveScroll: true,
        })
...
function toggleTagStatus(tag) {
    const wasActive = tag.is_active
    tag.is_active = !wasActive
    const url = wasActive ? route('admin.tags.deactivate', { id: tag.id }) : route('admin.tags.activate', { id: tag.id })
    router.patch(url, {}, {
        preserveState: true,
        preserveScroll: true,
        onError: () => {
...
const totalEntries = computed(() => props.tags?.total || filteredData.value.length)
const startEntry = computed(() => props.tags?.from || 1)
const endEntry = computed(() => props.tags?.to || filteredData.value.length)
const totalPages = computed(() => props.tags?.last_page || 1)
...
const toggleSelectAll = () => {
    filteredData.value.forEach((tag) => {
        tag.selected = selectAll.value
    })
}

const updateSelectAll = () => {
    const items = filteredData.value
    selectAll.value = items.length > 0 && items.every((tag) => tag.selected)
}
