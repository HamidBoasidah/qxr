      ...
      <table class="w-full min-w-full">
        ...
        <tbody>
          <tr v-if="paginatedData.length === 0">
            <td colspan="4" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400 border border-gray-100 dark:border-gray-800">
              {{ t('returnInvoice.noReturnInvoices') || 'لا توجد فواتير استرجاع' }}
            </td>
          </tr>
          <tr v-for="ri in paginatedData" :key="ri.id">
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ ri.original_invoice?.invoice_no || `#${ri.original_invoice_id}` }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400 font-medium">{{ priceLabel(ri.total_refund_amount) }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center gap-2">
                <Link :href="route('company.return-invoices.show', ri.id)" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
...
const filteredData = computed(() => {
  ...
  return allItems.value.filter(ri =>
    ...
    String(ri.original_invoice_id).includes(q)
  )
})
