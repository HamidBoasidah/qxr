  ...
  <div class="overflow-hidden">
    ...
    <div class="flex flex-col gap-2 px-4 py-4 border border-b-0 border-gray-200 rounded-b-none rounded-xl dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <Link :href="route('company.return-policies.create')" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 transition">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3.33337V12.6667M3.33337 8H12.6667" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" /></svg>
          {{ t('returnPolicy.createPolicy') || 'إنشاء سياسة' }}
        </Link>
      </div>
    </div>
      ...
      <table class="w-full min-w-full">
        <thead>
          <tr>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.name') || 'الاسم' }}</p>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.return_window_days') || 'نافذة الاسترجاع (أيام)' }}</p>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.max_return_ratio') || 'نسبة الاسترجاع القصوى' }}</p>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.is_default') || 'افتراضية' }}</p>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
        ...
        <tbody>
          <tr v-if="paginatedData.length === 0">
            <td colspan="6" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400 border border-gray-100 dark:border-gray-800">
              {{ t('returnPolicy.noPolicies') || 'لا توجد سياسات استرجاع' }}
            </td>
          </tr>
          <tr v-for="policy in paginatedData" :key="policy.id">
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400 font-medium">{{ policy.name }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ policy.return_window_days }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ (policy.max_return_ratio * 100).toFixed(0) }}%</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium" :class="{ 'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': policy.is_active, 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !policy.is_active }">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <!-- View -->
                <Link :href="route('company.return-policies.show', policy.id)" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90" :title="t('common.view') || 'عرض'">
                ...
                <!-- Edit -->
                <Link :href="route('company.return-policies.edit', policy.id)" class="text-gray-500 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-300" :title="t('common.edit') || 'تعديل'">
    ...
    <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div class="bg-white dark:bg-gray-900 rounded-2xl p-6 w-full max-w-sm shadow-xl">
        <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">{{ t('returnPolicy.deleteConfirmTitle') || 'حذف السياسة' }}</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">{{ t('returnPolicy.deleteConfirmMessage') || 'هل أنت متأكد من حذف هذه السياسة؟ لا يمكن التراجع عن هذا الإجراء.' }}</p>
        <div class="flex justify-end gap-3">
          <button @click="deleteTarget = null" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">{{ t('buttons.cancel') || 'إلغاء' }}</button>
          <button @click="executeDelete" :disabled="deleting" class="px-4 py-2 text-sm font-medium text-white bg-error-500 rounded-lg hover:bg-error-600 disabled:opacity-50">
...
const perPage = ref(props.policies.per_page || 10)
const search = ref('')
const currentPage = ref(1)

const allPolicies = computed(() => props.policies.data || [])

const filteredData = computed(() => {
  if (!search.value) return allPolicies.value
  const q = search.value.toLowerCase()
  return allPolicies.value.filter(p => p.name?.toLowerCase().includes(q))
})
...
function confirmDelete(policy) { deleteTarget.value = policy }

function executeDelete() {
  if (!deleteTarget.value) return
  deleting.value = true
  router.delete(route('company.return-policies.destroy', deleteTarget.value.id), {
    preserveScroll: true,
