  ...
  <div>
    <div class="p-5 mb-6 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
      <div class="flex items-center justify-between">
        <div>
          <h4 class="text-lg font-semibold text-gray-800 dark:text-white/90">{{ t('profile.labels.password') }}</h4>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('profile.changePasswordDesc') }}</p>
        </div>
        <button class="edit-button" @click="isPasswordModal = true">
    ...
    <Modal v-if="isPasswordModal" @close="closeModal">
      <template #body>
        <div class="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">{{ t('profile.changePassword') }}</h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">{{ t('profile.changePasswordDesc') }}</p>
          </div>
          <form class="flex flex-col px-2" @submit.prevent="savePassword">
            <div class="grid grid-cols-1 gap-y-5">
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ t('profile.labels.password') }}</label>
              ...
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ t('profile.labels.passwordConfirmation') }}</label>
                <input type="password" v-model="passwordForm.password_confirmation"
                  class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                  :placeholder="t('profile.labels.passwordConfirmation')"
...
const savePassword = () => {
  passwordForm.patch(updateRoute.value, {
    onSuccess: () => {
      closeModal()
