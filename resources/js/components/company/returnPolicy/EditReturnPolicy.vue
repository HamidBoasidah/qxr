      ...
      <div class="p-4 sm:p-6">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            ...
            <input v-model="form.name" type="text" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
          ...
          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ t('returnPolicy.max_return_ratio') || 'نسبة الاسترجاع القصوى' }} <span class="text-error-500">*</span></label>
          ...
          <div v-if="form.bonus_return_enabled">
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ t('returnPolicy.bonus_return_ratio') || 'نسبة استرجاع البونص' }} <span class="text-error-500">*</span></label>
...
const form = useForm({
  ...
  return_window_days: props.policy.return_window_days,
  max_return_ratio: props.policy.max_return_ratio,
  bonus_return_enabled: props.policy.bonus_return_enabled,
  bonus_return_ratio: props.policy.bonus_return_ratio,
  discount_deduction_enabled: props.policy.discount_deduction_enabled,
  min_days_before_expiry: props.policy.min_days_before_expiry,
  is_default: props.policy.is_default,
  is_active: props.policy.is_active,
})

function submit() {
  form.patch(route('company.return-policies.update', props.policy.id))
}
</script>
