...
function orderStatusLabel(status) {
  const labels = {
    draft: t('order.statuses.draft') || 'مسودة',
    pending: t('order.statuses.pending') || 'قيد الانتظار',
    approved: t('order.statuses.approved') || 'تمت الموافقة',
    preparing: t('order.statuses.preparing') || 'قيد التحضير',
    shipped: t('order.statuses.shipped') || 'تم الشحن',
    delivered: t('order.statuses.delivered') || 'تم التسليم',
    rejected: t('order.statuses.rejected') || 'مرفوض',
    cancelled: t('order.statuses.cancelled') || 'ملغي',
  };
  return labels[status] || status;
}
</script>
