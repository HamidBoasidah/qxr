<template>
	<div class="overflow-hidden">
		...
		<div class="max-w-full overflow-x-auto">
			<table class="w-full min-w-full">
					...
					<tr>
						...
						<th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
							<div
								...
								@click="sortBy('name_ar')"
							>
								<p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('governorates.nameAr') }}</p>
						...
						<th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
							<div
								class="flex items-center justify-between w-full cursor-pointer"
								@click="sortBy('name_en')"
							>
								<p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('governorates.nameEn') }}</p>
								<span class="flex flex-col gap-0.5">
									<svg
										class="fill-gray-300 dark:fill-gray-700"
										width="8"
				...
				<tbody>
					<tr
						v-for="governorate in paginatedData"
						:key="governorate.id"
						:class="{ 'bg-gray-50 dark:bg-gray-900': governorate.selected }"
					>
							...
							>
								<span class="relative">
									...
									<span
										...
										class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
									>
										<span :class="governorate.selected ? '' : 'opacity-0'">
											<svg
												width="12"
												height="12"
												viewBox="0 0 12 12"
						...
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ governorate.name_ar }}</p>
						</td>
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ governorate.name_en }}</p>
						</td>
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<span
								class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium"
								:class="{
									'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': governorate.is_active,
									'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !governorate.is_active,
								}"
							>
								{{ governorate.is_active ? t('common.active') : t('common.inactive') }}
							</span>
						</td>
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<label :for="'toggle-' + governorate.id" class="cursor-pointer">
								<div class="relative">
									<input
										type="checkbox"
										:id="'toggle-' + governorate.id"
										class="sr-only"
										:checked="governorate.is_active"
										@change="toggleGovernorateStatus(governorate)"
									/>
									<div
										class="block h-5 w-9 rounded-full"
										:class="governorate.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
									></div>
									<div
										:class="[governorate.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']"
						...
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<div class="flex items-center w-full gap-2">
								<Tooltip :text="t('messages.notAuthorized')" :show="!canView">
									<button
										:disabled="!canView"
										@click="handleViewClick(governorate.id)"
								...
								<Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
									<button
										:disabled="!canEdit"
										@click="handleEditClick(governorate.id)"
								...
								<Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
									<button
										:disabled="!canDelete"
										@click="handleDeleteClick(governorate.id)"
	...
	<DangerAlert
		...
		:message="t('messages.deleteGovernorateConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['governorates.create', 'governorates.store', 'governorates.add']))
const canView = computed(() => hasAnyPermission(['governorates.view', 'governorates.show', 'governorates.read']))
const canEdit = computed(() => hasAnyPermission(['governorates.update', 'governorates.edit']))
const canDelete = computed(() => hasAnyPermission(['governorates.delete', 'governorates.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ governorates: Object })

const search = ref('')
const sortColumn = ref('name_ar')
const sortDirection = ref('asc')
const currentPage = ref(props.governorates?.current_page ?? 1)
const perPage = ref(props.governorates?.per_page ?? 10)
const selectAll = ref(false)

function goToCreate() {
	router.visit(route('admin.governorates.create'))
}

function goToView(id) {
	router.visit(route('admin.governorates.show', id))
}

function goToEdit(id) {
	router.visit(route('admin.governorates.edit', id))
}
...
const governorateToDeleteId = ref(null)

function openDeleteModal(governorateId) {
	governorateToDeleteId.value = governorateId
	isDeleteModalOpen.value = true
}

function handleDeleteClick(governorateId) {
	if (!canDelete.value) return
	openDeleteModal(governorateId)
}

function closeDeleteModal() {
	isDeleteModalOpen.value = false
	governorateToDeleteId.value = null
}

function confirmDelete() {
	if (governorateToDeleteId.value) {
		router.delete(route('admin.governorates.destroy', governorateToDeleteId.value), {
			onSuccess: () => {
				success(t('governorates.governorateDeletedSuccessfully'))
				closeDeleteModal()
			},
			onError: () => {
				error(t('governorates.governorateDeletionFailed'))
				closeDeleteModal()
			},
			preserveScroll: true,
		})
...
function toggleGovernorateStatus(governorate) {
	const wasActive = governorate.is_active
	governorate.is_active = !wasActive
	const url = wasActive
		? route('admin.governorates.deactivate', { id: governorate.id })
		: route('admin.governorates.activate', { id: governorate.id })
	router.patch(url, {}, {
		preserveState: true,
		preserveScroll: true,
		onError: () => {
...
const filteredData = computed(() => {
	...
	return (props.governorates?.data || [])
		.filter((governorate) => {
			const nameAr = governorate.name_ar?.toLowerCase() || ''
			const nameEn = governorate.name_en?.toLowerCase() || ''
			return nameAr.includes(searchLower) || nameEn.includes(searchLower)
		})
		.sort((a, b) => {
...
const totalEntries = computed(() => props.governorates?.total || filteredData.value.length)
const startEntry = computed(() => props.governorates?.from || 1)
const endEntry = computed(() => props.governorates?.to || filteredData.value.length)
const totalPages = computed(() => props.governorates?.last_page || 1)
...
watch(
	() => props.governorates?.current_page,
	(val) => {
...
const toggleSelectAll = () => {
	filteredData.value.forEach((governorate) => {
		governorate.selected = selectAll.value
	})
}

const updateSelectAll = () => {
	const items = filteredData.value
	selectAll.value = items.length > 0 && items.every((governorate) => governorate.selected)
}
