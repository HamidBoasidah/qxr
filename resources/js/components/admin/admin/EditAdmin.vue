	...
	<div class="space-y-6">
		...
		<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
			...
			<div class="p-4 sm:p-6">
				<label
					for="admin-avatar"
					class="group relative block cursor-pointer rounded-lg border-2 border-dashed border-gray-300 transition hover:border-brand-500 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-brand-500"
				>
				...
				<p v-if="form.errors.avatar" class="mt-2 text-sm text-error-500">{{ form.errors.avatar }}</p>
			</div>
		</div>
...
const originalAvatarUrl = props.admin?.avatar ? `/storage/${props.admin.avatar}` : null
const imagePreview = ref(originalAvatarUrl)
...
const handleFileUpload = (event) => {
	...
	form.avatar = file
	imagePreview.value = file ? URL.createObjectURL(file) : originalAvatarUrl
}
