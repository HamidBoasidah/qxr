<template>
  <div class="space-y-6">
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div class="p-4 sm:p-6">
        <div class="grid grid-cols-1 gap-x-5 gap-y-6 md:grid-cols-2">
          ...
          <InfoItem :label="t('users.company') || 'الشركة'" :value="policy.company ? `${policy.company.first_name} ${policy.company.last_name}` : `#${policy.company_id}`" />
          <InfoItem :label="t('common.status')">
            <Badge :color="policy.is_active ? 'success' : 'dark'" variant="light" size="sm">{{ policy.is_active ? (t('common.active') || 'نشطة') : (t('common.inactive') || 'غير نشطة') }}</Badge>
          </InfoItem>
          ...
          <InfoItem :label="t('returnPolicy.discount_deduction_enabled') || 'خصم الاسترجاع'">
            <Badge :color="policy.discount_deduction_enabled ? 'info' : 'light'" variant="light" size="sm">{{ policy.discount_deduction_enabled ? (t('common.yes') || 'نعم') : (t('common.no') || 'لا') }}</Badge>
          </InfoItem>
          ...
          <InfoItem :label="t('returnPolicy.bonus_return_enabled') || 'استرجاع البونص'">
            <Badge :color="policy.bonus_return_enabled ? 'info' : 'light'" variant="light" size="sm">{{ policy.bonus_return_enabled ? (t('common.yes') || 'نعم') : (t('common.no') || 'لا') }}</Badge>
          </InfoItem>
    ...
    <div class="flex justify-end">
      <Link :href="route('admin.return-policies.index')" class="shadow-theme-xs inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 ring-1 ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">
