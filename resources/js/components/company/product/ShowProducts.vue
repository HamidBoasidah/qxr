      ...
      <table class="w-full min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('sku')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('product.sku') }}</p>
                <SortArrows />
              </div>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('category_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('product.Category') }}</p>
                <SortArrows />
              </div>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('base_price')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('product.base_price') || 'Base Price' }}</p>
                <SortArrows />
              </div>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('product.unit') || 'Unit' }}</p>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
        ...
        <tbody>
          <tr
            ...
            :key="product.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': product.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="product.selected" @change="updateSelectAll" />
                  <span
                    :class="product.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="product.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </span>
              ...
              <div class="flex items-center gap-3">
                <div class="h-10 w-10">
                  <img
                    ...
                    class="h-10 w-10 rounded-none object-contain"
                    alt=""
                  />
                  <ProductIcon v-else class="h-10 w-10 text-gray-400" />
                </div>
                <div class="flex flex-col">
                  <span class="text-sm font-medium text-gray-700 dark:text-gray-400">{{ product.name }}</span>
                </div>
              </div>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ product.sku || '—' }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ categoryLabel(product) }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ companyLabel(product) }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ priceLabel(product.base_price) }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ product.unit_name || '—' }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span
...
// usePermissions removed for server-side enforced authorization
import { useNotifications } from '@/composables/useNotifications'
import SortArrows from '@/components/common/SortArrows.vue'

// permission checks removed — server enforces authorization
...
function goToCreate() {
  router.visit(route('company.products.create'))
}

function goToView(id) {
  router.visit(route('company.products.show', id))
}

function goToEdit(id) {
  router.visit(route('company.products.edit', id))
}

function handleCreateClick() { goToCreate() }

function handleViewClick(id) { goToView(id) }

function handleEditClick(id) { goToEdit(id) }
...
function handleDeleteClick(productId) { openDeleteModal(productId) }
...
function confirmDelete() {
  if (productToDeleteId.value) {
    router.delete(route('company.products.destroy', productToDeleteId.value), {
      onSuccess: () => {
...
function toggleProductStatus(product) {
  ...
  const routeName = product.is_active ? 'company.products.activate' : 'company.products.deactivate'
...
const filteredData = computed(() => {
  ...
  return (props.products?.data || [])
    .filter((product) => {
      const categoryName = categoryLabel(product).toLowerCase()
      const companyName = companyLabel(product).toLowerCase()
      return (
    ...
    .sort((a, b) => {
      ...
      // computed labels for sorting category/company
      if (col === 'category_name') {
        aVal = categoryLabel(a)
        bVal = categoryLabel(b)
      }
      if (col === 'company_name') {
        aVal = companyLabel(a)
        bVal = companyLabel(b)
...
const totalEntries = computed(() => props.products?.total || filteredData.value.length)
...
const toggleSelectAll = () => {
  filteredData.value.forEach((product) => (product.selected = selectAll.value))
}

const updateSelectAll = () => {
  const items = filteredData.value
  selectAll.value = items.length > 0 && items.every((product) => product.selected)
}
...
function categoryLabel(product) {
  ...
  if (!category) return product.category_name || '—'
  const loc = locale.value
  return category?.name?.[loc] ?? category?.name_ar ?? category?.name_en ?? category?.name ?? product.category_name ?? '—'
}

function companyLabel(product) {
  const company = product.company || product.company_user
  if (!company) return '—'
  return company.company_name || `${company.first_name || ''} ${company.last_name || ''}`.trim() || company.email || '—'
}
