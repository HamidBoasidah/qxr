<template>
          ...
          <tr v-for="offer in paginatedData" :key="offer.id">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                ...
                <button
                  @click="handleEditClick(offer.id)"
                ...
                <button
                  @click="handleDeleteClick(offer.id)"
  ...
  <DangerAlert
    ...
    :message="t('offers.deleteConfirmation')"
...
function handleCreateClick() {
  router.visit(route('company.offers.create'))
}

function handleViewClick(id) {
  router.visit(route('company.offers.show', id))
}

function handleEditClick(id) {
  router.visit(route('company.offers.edit', id))
}

function handleDeleteClick(offerId) {
...
function confirmDelete() {
  if (offerToDeleteId.value) {
    router.delete(route('company.offers.destroy', offerToDeleteId.value), {
      onSuccess: () => {
        success(t('offers.offerDeletedSuccessfully'))
        closeDeleteModal()
      },
      onError: () => {
        error(t('offers.offerDeletionFailed'))
        closeDeleteModal()
      },
      preserveScroll: true,
    })
