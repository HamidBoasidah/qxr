<template>
  <div class="overflow-hidden">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <Tooltip :text="t('messages.notAuthorized')" :show="!canCreate">
          <button
            ...
            class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-[11px] text-sm font-medium text-white transition sm:w-auto disabled:bg-brand-300 disabled:text-white/70 disabled:hover:bg-brand-300"
          >
      ...
      <table class="w-full min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('permissions_count')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('roles.permissionsCountShort') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill="" />
                  </svg>
        ...
        <tbody>
          <tr v-for="role in paginatedData" :key="role.id" :class="{ 'bg-gray-50 dark:bg-gray-900': role.selected }">
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="role.selected" @change="updateSelectAll" />
                  <span :class="role.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]">
                    <span :class="role.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center gap-3">
                <div class="flex flex-col">
                  ...
                  <!-- اختياري: إظهار السلَغ للمرجع التقني -->
                  <!-- <span class="text-xs text-gray-400 dark:text-gray-500">{{ role.name }}</span> -->
                </div>
              </div>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span class="inline-flex items-center rounded-full bg-brand-50 dark:bg-brand-500/15 px-2.5 py-0.5 text-xs font-medium text-brand-600 dark:text-brand-400">{{ role.permissions_count }}</span>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
    ...
    <DangerAlert
      ...
      :message="t('messages.deleteRoleConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['roles.create', 'roles.store', 'roles.add']))
const canView = computed(() => hasAnyPermission(['roles.view', 'roles.show', 'roles.read']))
const canEdit = computed(() => hasAnyPermission(['roles.update', 'roles.edit']))
const canDelete = computed(() => hasAnyPermission(['roles.delete', 'roles.destroy']))
...
// helpers
const displayName = (r) => {
  // ترتيب fallback: اللغة الحالية -> en -> name
  const loc = locale.value
  return r?.display_name?.[loc] ?? r?.display_name?.en ?? r?.name ?? ''
}

// actions
function openDeleteModal(id){ roleToDeleteId.value = id; isDeleteModalOpen.value = true }
function closeDeleteModal(){ isDeleteModalOpen.value = false; roleToDeleteId.value = null }
function confirmDelete(){
  if(roleToDeleteId.value && canDelete.value){
    router.delete(route('admin.roles.destroy', roleToDeleteId.value), {
      onSuccess:()=>{ success(t('roles.roleDeletedSuccessfully')); closeDeleteModal() },
      onError:()=>{ error(t('roles.roleDeletionFailed')); closeDeleteModal() },
      preserveScroll:true
    })
  }
}
function goToCreate(){ router.visit(route('admin.roles.create')) }
function goToEdit(id){ router.visit(route('admin.roles.edit', id)) }
function goToShow(id){ router.visit(route('admin.roles.show', id)) }
...
const filteredData = computed(() => {
  ...
  return (props.roles?.data || [])
    .filter(r => displayName(r).toLowerCase().includes(searchLower))
    .sort((a, b) => {
      const aVal = sortColumn.value === 'name' ? displayName(a) : (a[sortColumn.value] ?? '')
      const bVal = sortColumn.value === 'name' ? displayName(b) : (b[sortColumn.value] ?? '')
...
const totalEntries = computed(() => props.roles?.total || filteredData.value.length)
const startEntry = computed(() => props.roles?.from || 1)
const endEntry = computed(() => props.roles?.to || filteredData.value.length)
...
// watchers
watch(() => props.roles?.current_page, v => { currentPage.value = typeof v === 'number' ? v : 1 })
watch(() => props.roles?.per_page, v => { if (typeof v === 'number') perPage.value = v })
...
const goToPage = (p) => { if (p >= 1 && p <= totalPages.value) fetchPage(p) }
...
const toggleSelectAll = () => { filteredData.value.forEach(r => r.selected = selectAll.value) }
const updateSelectAll = () => {
  const items = filteredData.value
  selectAll.value = items.length > 0 && items.every(r => r.selected)
}
</script>
