    ...
    <div class="space-y-6">
      ...
      <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div v-if="localFilters.date_preset === 'custom'" class="md:col-span-2 grid grid-cols-2 gap-4">
            <div>
              <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                {{ t('common.from') }}
              </label>
          ...
          <!-- Company Filter -->
          <div :class="{ 'md:col-span-2': localFilters.date_preset !== 'custom' }">
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ t('users.company') }}
            </label>
            <select 
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KPICard 
          :title="t('menu.users')"
          :value="stats.kpis.users.total_users"
          icon="👥"
          color="indigo"
        >
          <template #details>
            <div class="mt-2 space-y-1 text-xs">
              <div class="flex justify-between">
                <span>{{ t('users.customers') }}:</span>
                <span class="font-semibold">{{ stats.kpis.users.total_customers }}</span>
              </div>
              <div class="flex justify-between">
                <span>{{ t('users.companies') }}:</span>
                <span class="font-semibold">{{ stats.kpis.users.total_companies }}</span>
              </div>
            </div>
          </template>
        </KPICard>
        ...
        >
          <template #details>
            <div class="mt-2 space-y-1 text-xs">
              ...
              <div class="flex justify-between">
                <span>{{ t('offers.public') }}:</span>
                <span class="font-semibold">{{ stats.kpis.offers.public }}</span>
              </div>
              <div class="flex justify-between">
                <span>{{ t('offers.private') }}:</span>
                <span class="font-semibold">{{ stats.kpis.offers.private }}</span>
              </div>
            </div>
          </template>
        </KPICard>
          ...
          <template #details>
            <div class="mt-2 space-y-1 text-xs">
              <div class="flex justify-between">
                ...
                <span class="font-semibold">{{ stats.kpis.products.active }}</span>
              </div>
              <div class="flex justify-between">
                <span>{{ t('common.inactive') }}:</span>
                <span class="font-semibold">{{ stats.kpis.products.inactive }}</span>
              </div>
            </div>
          </template>
        </KPICard>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <!-- Top Companies Chart -->
        <ChartCard :title="t('dashboard.top_companies')">
          <BarChart 
            :chartData="stats.charts.top_companies"
        ...
        <!-- Top Products Chart -->
        <ChartCard :title="t('dashboard.top_products')">
          <BarChart 
...
console.log('Dashboard Stats:', {
  kpis: props.stats.kpis,
  chartsKeys: Object.keys(props.stats.charts),
  ordersOverTime: props.stats.charts.orders_over_time,
  revenueOverTime: props.stats.charts.revenue_over_time
})
...
const onFilterChange = () => {
  ...
  if (localFilters.value.company_id) {
    filters.company_id = localFilters.value.company_id
  }
