  ...
  <div class="overflow-hidden">
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
        <thead>
          <tr>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800"><p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.name') || 'الاسم' }}</p></th>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800"><p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.return_window_days') || 'نافذة الاسترجاع' }}</p></th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800"><p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.max_return_ratio') || 'نسبة الاسترجاع' }}</p></th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800"><p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('returnPolicy.is_default') || 'افتراضية' }}</p></th>
        ...
        <tbody>
          <tr v-if="paginatedData.length === 0">
            <td colspan="7" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400 border border-gray-100 dark:border-gray-800">{{ t('returnPolicy.noPolicies') || 'لا توجد سياسات' }}</td>
          </tr>
          <tr v-for="policy in paginatedData" :key="policy.id">
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800"><p class="text-gray-700 text-theme-sm dark:text-gray-400 font-medium">{{ policy.name }}</p></td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800"><p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ policy.company ? `${policy.company.first_name} ${policy.company.last_name}` : `#${policy.company_id}` }}</p></td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800"><p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ policy.return_window_days }}</p></td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800"><p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ (policy.max_return_ratio * 100).toFixed(0) }}%</p></td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <Badge :color="policy.is_default ? 'success' : 'light'" variant="light" size="sm">{{ policy.is_default ? (t('common.yes') || 'نعم') : (t('common.no') || 'لا') }}</Badge>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <Link :href="route('admin.return-policies.show', policy.id)" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
...
const { t } = useI18n()
const props = defineProps({ policies: { type: Object, required: true } })
...
const filteredData = computed(() => {
  ...
  return allPolicies.value.filter(p =>
    ...
    `${p.company?.first_name} ${p.company?.last_name}`.toLowerCase().includes(q)
  )
})
